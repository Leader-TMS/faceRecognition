...
#             # Lấy điểm mũi (landmark số 1 là đỉnh mũi trong FaceMesh)
#             nose = face_landmarks.landmark[1]
#             cx, cy = int(nose.x * w), int(nose.y * h)

#             # Vẽ hình tròn lên mũi
#             cv2.circle(frame, (cx, cy), 10, (0, 255, 0), -1)

#             if not tracking:
#                 # Định nghĩa vùng theo dõi xung quanh mũi
#                 x, y, w_box, h_box = cx - 20, cy - 20, 40, 40
...
#     cv2.imshow('Tracking Nose', frame)
...
faceJobStates = {}  # objectId → {step, result, is_running, last_frame}
trackName = {}      # objectId → final name (if passed)
faceSpeedTrackers = {}
objTypeText = {}
objCheckName = {}
...
#     def __init__(self, maxDisappeared=1, iouThreshold=0.5):
#         self.nextObjectId = 0
...
#                 if self.isBoxBInsideBoxA(scaledTrackedBox, newBox):
#                     self.objects[objectId] = newBox 
...
#                 # scaledBox = self.scaleBox(newBox)
...
def faceRecognition(faceImage):
    try:
        ...
        if faceImage is not None and faceImage.size > 0:
            ...
            print(f'faceMtcnn: {faceMtcnn}')
            print(f'len: {len(faceMtcnn)}')
...
def runRecognitionJob(objectId):
    ...
    if state["step"] < len(state["list_face"]) and state["list_face"][state["step"]] is not None:
        ...
        state["result"].append(name)
        state["code"] = code
        state["step"] += 1
        state["last_frame"] = faceImage
            ...
            if len(objects):
                for objectId, (x, y, w, h) in objects.items():
                    if objectId not in faceJobStates:
                        faceJobStates[objectId] = {"step": 0, "result": [], "code": None, "last_frame": None, "success": False, "list_face": []}
                    state = faceJobStates[objectId]
                    filteredIds = [objId for objId, data in faceJobStates.items() if data['success'] == False]
                    minId = min(filteredIds) if filteredIds else None
                    cv2.putText(frame, f'objectId: {objectId} - minId: {minId}', (x - 50, int(y - 50)), cv2.FONT_HERSHEY_DUPLEX, 0.8, Color.Red, 2)
                    
                    if objectId == minId and not startThread.is_alive():
                                ...
                                if state["step"] < countThread and not startThread.is_alive():
                                    if angle and lowSpeed:
                                        if blur:
                                            ...
                                            faceCrop = imgOpt
                                        if not blur:
                                            state["list_face"].append(faceCrop.copy())
                                            startThread = threading.Thread(
                                                target=runRecognitionJob,
                                                args=(objectId, )
                                            )
                                            startThread.daemon = True
                                            startThread.start()
                                    ...
                                    if len(state["list_face"]) < countThread:
                                        if light and angle and lowSpeed:
                                            if blur:
                                                imgOpt = motionBlurCompensation(faceCrop)
                                                blur = checkBlurry(imgOpt)
                                                faceCrop = imgOpt
                                    ...
                                    if len(names) == countThread and names.count(None) == 0 and len(set(names)) == 1:
                                        ...
                                        log(f"[ID {objectId}] Lỗi: {names} → reset lại")
                                        faceJobStates[objectId] = {
                                            "step": 0,
                                            "result": [],
                                            "code": None,
                                            "last_frame": None,
